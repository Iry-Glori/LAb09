...
int alpha = 0, beta = 0; // оголошуємо змінні - кути повороту

// ця функція викликається кожного разу, коли надходить повідомлення від миші
void CALLBACK mouse(AUX_EVENTREC *event)
{
	static int x0, y0 = -12345; // в цих двох змінних будемо зберігати старі положення миші
	...
	x0 = event->data[AUX_MOUSEX]; // зберігаємо положення миші
	y0 = event->data[AUX_MOUSEY];
}

// далі - функції-обробники стрілок клавіатури
